...
internal static class UnderlyingType
{
    ...
    {
        if (sort is not DatatypeSort datatypeSort)
        {
            return false;
        }

        if (datatypeSort.NumConstructors is not 2)
        {
            return false;
        }
    ...
    public static bool IsEnumerable(Sort sort)
    {
        ...
        if (datatypeSort.NumConstructors is not 1)
        {
            return false;
        }

        var createEnumerable = datatypeSort.Constructors[0];
        if (createEnumerable.Arity is not 1)
        {
            return false;
        }
